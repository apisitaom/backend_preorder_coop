...
async function lists (req, res) {
    ...
    const sql = `
        select 
            ...
            member.firstname, member.lastname, orderproduct.orderid,
...
async function add (req, res) {
    ...
    const active = true;    
        if(!req.files[0]){
            ...
            const valuePayment = [active, moment(date).format('YYYY-MM-DD HH:mm:ss'), total, req.files[0].filename, 2, payid];    
...
async function adminpaymentlists (req, res) {
    const sql = `
        select 
            orderdetail.orderdetailid, orderdetail.createdate, orderdetail.proopids, orderdetail.amounts,
            orderdetail.address, orderdetail.disstrict, orderdetail.province, orderdetail.zipcode, orderdetail.phone,
...
async function adminpaymentcheck (req, res) {
    const sql = `
        select 
...
async function adminpaymentadd (req, res) {
