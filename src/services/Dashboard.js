...
async function sales (req, res) {
    let resp
    let sql = `select cast(sum(grand_price) as integer) from receipt_detail`
    try {
        if(req.params.id){
            const value = [req.params.id] 
...
async function totalAmount (req, res) {
    let sql = `select cast(sum(amount) as integer) from receipt_detail `
    try {
        if(req.params.id){
            sql += 'where seller_id = $1'
            const value = [req.params.id]
...
async function totalSeller (req, res) {
    const sql = `select count(sellerid) as sum from seller where active = true`
    try {
        const { rows } = await db.query(sql)
        return Response.resSuccess(res, successMessage.success, rows[0]);
    } catch (error) {
